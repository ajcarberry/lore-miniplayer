...
- **Diff/merge operations**: `fileDiff` drives the Project View's compare (`DiffService`); `branchMergeStart/ResolveMine/ResolveTheirs/Abort/Into` drive its merge workflow (`MergeService`, one in-flight merge per checkout)
- **Project View**: the full-page review/merge surface the card morphs into IN the main window (`window:setView` resizes to the review footprint, drops always-on-top, and restores the card's bounds on exit); opened from the WorkingSet header's single gated entry (Review when dirty; Merge when clean with revisions to land) or the footer's always-visible icon; a header switcher crosses between the commit and merge workflows (Merge segment disabled while ANY work is uncommitted — merge aborts reset the working tree, and the service refuses to start over dirt), and exits are the header's Back (→ card) or the TitleBar collapse (→ pill)
