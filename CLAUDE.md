...
- **Project View**: the full-page review/merge surface the card morphs into IN the main window (`window:setView` resizes to the review footprint, drops always-on-top, and restores the card's bounds on exit); opened from the WorkingSet header's gated Review/Merge actions or the footer's always-visible icon, backed out via the header's Back (→ card) or the TitleBar collapse (→ pill)
