...
import { test, expect } from './support/ui';
import type { ReviewOpenRequest } from '../../../src/shared/types';

// Requires `pnpm build` first — launches the built app at out/main/index.js.
//
// Exercises the open-review seam directly: `window.electronAPI.review.open`
// is the exact call `requestOpenReviewWindow` (review/openReview.ts) makes
// from the card's Review / Merge actions, so invoking it from the test with a
// fabricated `ReviewOpenRequest` reaches the real IPC handler and window
// (src/main/ipc/window-handlers.ts's registerReviewWindow) without needing a
// connected card to click through. `ReviewWindow.tsx` routes purely on
// `request.workflow` ('commit' | 'merge'), independent of whether the
// underlying repository/diff calls succeed — so a repositoryPath that isn't a
// real Lore repository is enough to prove the routing without a live server:
// the commit view always renders its bottom bar, and the merge view surfaces
// its own "could not start" error state for a non-repository path rather than
// silently falling back to the commit view.

function makeRepositoryDir(): string {
...
test.describe('Review window — reviewIntent seam routing', () => {
  test('a commit-workflow request opens the commit view', async ({ electronApp, window }) => {
    const repositoryPath = makeRepositoryDir();
    ...
    const request: ReviewOpenRequest = {
      ...
      workflow: 'commit',
      compare: {
        source: { kind: 'branchHead', branch: 'feat/my-task' },
  ...
  test('a merge-workflow request opens the merge view', async ({ electronApp, window }) => {
    const repositoryPath = makeRepositoryDir();
